...
`dbt/` (project `dbt_ie_supermarket_analytics`, profile of the same name) transforms the raw scraped data using **dbt-duckdb** — DuckDB reads the gzipped JSONL directly from S3 via `httpfs`, no warehouse load step. Run commands with `uv run dbt <command> --project-dir dbt` (requires the `dbt` group; a local `profiles.yml` with AWS credentials configured for the `dbt_ie_supermarket_analytics` profile is not checked in). Note: the `dbt` group's `dbt-core`/`dbt-common` pin currently doesn't import under this repo's `==3.14.*` Python (a `mashumaro` `UnserializableField` error) — run dbt from a scratch venv on Python ≤3.13 until that's fixed. A `dbt-fusion` binary is also available on this machine, but as of `2.0.0-preview.202` it doesn't support the `external_location`-based source config this project uses (see `models/staging/raw_s3.yml`), so stick to dbt-core for now.
