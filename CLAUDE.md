...
`dbt/` (project `dbt_ie_supermarket_analytics`, profile of the same name) transforms the raw scraped data using **dbt-duckdb** — DuckDB reads the gzipped JSONL directly from S3 via `httpfs`, no warehouse load step. The project runs on **dbt Fusion** (`dbt` on PATH, currently `2.0.0-preview.202`); `uv sync --group dbt` (dbt-core/dbt-duckdb) is also available as a fallback but its `dbt-common`/`mashumaro` pin doesn't import under this repo's `==3.14.*` Python — use a scratch venv on Python ≤3.13 if you need it. As of `2.0.0-preview.202`, Fusion's `dbt show`/inline queries and `dbt compile` resolve the `external_location`-based sources in `models/staging/raw_s3.yml` correctly (confirmed against real S3 data), but `dbt run`/`dbt build` fail to materialize the staging models (`CREATE VIEW`/`CREATE TABLE`) from those same sources — it appears to resolve the literal S3 glob path as a relative schema-qualified object name rather than a file path during materialization specifically. If `dbt run`/`build` errors with `TableNotFound`/`does not exist in schema ...` on a staging model, that's this — fall back to dbt-core for actual runs/builds until it's fixed upstream.
