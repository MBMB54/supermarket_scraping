...
class CategoryMatch(BaseModel):
    reasoning: str = Field(
        description="Brief justification for the chosen category, or why none of the candidates fit"
    )
    selected_index: Literal[0, 1, 2, 3, 4, -1] = Field(
        description="Index of the matching candidate category, or -1 if none apply"
    )


SYSTEM_PROMPT = """You are a supermarket retail product matching expert.

You will be given a product title from an Irish supermarket, and a list of 5 candidate \
category pairs from SuperValu (the reference taxonomy), ranked by cosine similarity of \
product title embeddings, most similar first. Each candidate has a category_2 (parent) \
and category_3 (subcategory).

Your task is to select the SINGLE candidate that best matches the product, using its index \
(0-4). Consider the full product title carefully, including any qualifiers \
that might disambiguate between plausible categories.

Similarity ranking is a hint, not a guarantee: a lower-ranked candidate can still be the \
correct match if it fits the product better, and higher-ranked candidates can be wrong if \
they reflect a superficial embedding match rather than a true category fit.

If none of the 5 candidates are a reasonable fit for the product, return -1. Do not force a \
choice among clearly unsuitable options.

Respond with brief reasoning followed by the selected index."""


HARMONY_FINAL_CHANNEL_MARKER = "<|channel|>final<|message|>"


def parse_structured_response(response_text: str) -> tuple[str, int | None]:
    """Parse a CategoryMatch JSON response into (reasoning, selected_index).

    Harmony-format models (e.g. gpt-oss) emit hidden analysis-channel reasoning
    before the final-channel answer in the same completion text; only the
    content after the final-channel marker is the structured JSON we want.
    """
    if not response_text:
        return "", None
    if HARMONY_FINAL_CHANNEL_MARKER in response_text:
        response_text = response_text.rsplit(HARMONY_FINAL_CHANNEL_MARKER, 1)[-1]
    response_text = response_text.split("<|return|>")[0].split("<|end|>")[0].strip()
    try:
        parsed = CategoryMatch.model_validate_json(response_text)
        return parsed.reasoning, parsed.selected_index
    except ValidationError:
        logger.warning(f"Failed to parse structured response: {response_text[:200]!r}")
...
- `response`: The raw structured JSON response from the model
- `reasoning`: The model's reasoning for its category selection
- `selected_index`: Index (0-4) of the chosen candidate category, or -1 if none fit
- `selected_category_2`: The chosen candidate's category_2 value, or null if no match
- `selected_category_3`: The chosen candidate's category_3 value, or null if no match
## Generation Script
...
def main(
    ...
    candidates_column: str = "category_2",
    ...
    Args:
        ...
        candidates_column: Column name containing the list of candidate category_2/category_3
            dicts that selected_index refers to
    ...
    sampling_params = SamplingParams(
        ...
        structured_outputs=StructuredOutputsParams(json=CategoryMatch.model_json_schema()),
    ...
    # gpt-oss's harmony format takes an explicit "Reasoning: low" directive in the
    # system prompt; other models don't recognize this convention, so only add it
    # for gpt-oss to keep reasoning effort (and hidden analysis-channel output) low.
    ...
    # Parse the structured output into reasoning + selected_index, and resolve
    # selected_index against each row's own candidate list to get the chosen category
    logger.info("Parsing structured responses and resolving selected categories...")
    reasonings = []
    selected_indices = []
    selected_category_2s = []
    selected_category_3s = []

    for i, response in enumerate(responses):
        reasoning, selected_index = parse_structured_response(response)
        reasonings.append(reasoning)
        selected_indices.append(selected_index)

        candidates = dataset[i][candidates_column]
        if selected_index is not None and 0 <= selected_index < len(candidates):
            selected_category_2s.append(candidates[selected_index]["category_2"])
            selected_category_3s.append(candidates[selected_index]["category_3"])
        else:
            selected_category_2s.append(None)
            selected_category_3s.append(None)

    dataset = dataset.add_column("reasoning", reasonings)
    dataset = dataset.add_column("selected_index", selected_indices)
    dataset = dataset.add_column("selected_category_2", selected_category_2s)
    dataset = dataset.add_column("selected_category_3", selected_category_3s)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        parser = argparse.ArgumentParser(
            ...
            epilog="""
Examples:
  # Basic usage with default gpt-oss model
  ...
  HF_TOKEN=hf_xxx uv run generate-responses.py input-dataset output-dataset
        ...
        parser.add_argument(
            ...
            help="Model to use for generation (default: gpt-oss-20b)",
        )
        parser.add_argument(
            "--messages-column",
            type=str,
        ...
        parser.add_argument(
            ...
            type=str,
            default="category_2",
            help="Column containing the candidate category_2/category_3 list that "
            "selected_index refers to (default: category_2)",
        )
        parser.add_argument(
            "--max-samples",
            type=int,
