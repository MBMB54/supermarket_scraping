...
class OcadoScraper:
    ...
    def extract_page_data(self, driver: webdriver.Chrome) -> tuple[list[str], list[str], list[str]]:
        ...
        for container in containers:
            try:
                name = (
                    container.find_element(By.CSS_SELECTOR, "h4.fop-title")
                    .get_attribute("title")
                    .strip()
                )
    ...
    def scrape_category(self, category: str) -> pd.DataFrame:
        ...
        try:
            ...
            for name, price, weight in zip(names, prices, weights, strict=False):
                all_data.append(
                    {"product_name": name, "price": price, "weight": weight, "category": category}
                )
...
def main():
    categories = [
        "frozen-303714",
        "best-of-fresh-294566",
        "food-cupboard-drinks-bakery-294572",
