...
logger = logging.getLogger("dunnes_api")
...
DUNNES_IDS = (
    pl.read_parquet(
        f"s3://{BUCKET}/raw/dunnes/ids/latest/dunnes_product_ids.parquet",
...
logger.info(f"Loaded {len(DUNNES_IDS)} dunnes product IDs from S3")
...
def fetch_store_ids() -> list[str]:
    headers = {
        ...
        "x-site-host": "https://www.dunnesstoresgrocery.com",
        "x-site-location": "HeadersBuilderInterceptor",
        "x-correlation-id": str(uuid.uuid4()),
        "x-shopping-mode": "11111111-1111-1111-1111-111111111111",
        "x-customer-session-id": f"https://www.dunnesstoresgrocery.com|{uuid.uuid4()}",
        "Referer": "https://www.dunnesstoresgrocery.com/",
    }
    response = requests.get(
        "https://storefrontgateway.dunnesstoresgrocery.com/api/stores", headers=headers, timeout=30
    )
...
logger.info(f"Loaded {len(_all_store_ids)} dunnes store IDs")
...
def load_checkpoint(chunk_id: int, folder_date: str) -> set[str]:
    s3 = boto3.client("s3")
    key = f"raw/dunnes/{folder_date}/checkpoints/chunk_{chunk_id}.json"
    try:
        ...
        for store_id in STORE_IDS:
            try:
                async with session.get(
                    f"https://storefrontgateway.dunnesstoresgrocery.com/api/stores/{store_id}/products/{product_id}",
                    headers=get_headers(),
                ) as response:
...
async def fetch_all_products(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for batch_num, i in enumerate(range(0, len(tasks), batch_size)):
            ...
            batch_filename = f"dunnes_raw_{timestamp}_chunk{chunk_id}_batch{batch_num:04d}.jsonl.gz"
            tmp_path = f"/tmp/{batch_filename}"
            with gzip.open(tmp_path, "wt", encoding="utf-8") as f:
                for result in batch_results:
                    f.write(json.dumps(result) + "\n")
            s3.upload_file(tmp_path, BUCKET, f"raw/dunnes/{folder_date}/{batch_filename}")
...
chunk_size = len(DUNNES_IDS) // total_chunks
start = chunk_id * chunk_size
end = start + chunk_size if chunk_id < total_chunks - 1 else len(DUNNES_IDS)
product_ids = DUNNES_IDS[start:end]
